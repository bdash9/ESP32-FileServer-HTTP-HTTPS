// Found this code at https://github.com/stooged/PS5-Server32
// Adapted for the Adafruit ESP32-s2 feather tft by Ben Dash
...
void setup()
{
  ...
  tft.println("HTTPS File Server");
  tft.println();
  tft.setTextColor(ST77XX_WHITE);
  tft.println("Version: " + firmwareVer);
...
if (!SPIFFS.begin()) {
  ...
  else
  {
    ...
    tft.println("Filesystem failed to mount");
  ...
  if (connectWifi && WIFI_SSID.length() > 0 && WIFI_PASS.length() > 0)
  {
    ...
    else
    {
      ...
      if (LAN_IP)
      {
        ...
        tft.println("WEB Server LAN IP:");
        tft.println(LAN_IP.toString());
        tft.println("WEB Server Hostname: " + WIFI_HOSTNAME);
        delay(1000);
        ...
        if (!startAP)
        {
          ...
          tft.println("DNS server started");
          tft.println("DNS Server IP: " + LAN_IP.toString());
          delay(1000);          
          tft.fillScreen(ST77XX_BLACK);
          tft.setCursor(0, 0);          
        }
  ...
  if (FILESYS.exists("/pk.pem") && FILESYS.exists("/cert.der"))
  {
    ...
    if (createCertResult != 0) {
      ...
      tft.printf("Certificate failed, Error Code = 0x%02X\n", createCertResult);
